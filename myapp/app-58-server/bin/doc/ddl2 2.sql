...
-- 게시판 작성자에 대해 외부키 설정
alter table myapp_board
  add constraint myapp_board_fk1 foreign key (writer) references myapp_member (member_no);

alter table myapp_board
  add constraint myapp_board_fk2 foreign key (writer) references myapp_patient (patient_no);
